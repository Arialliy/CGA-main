...
\title{Component-Geometry Regularization for Infrared Small Target Detection}
...
\begin{abstract}
Infrared small target detection requires recovering tiny, low-contrast objects from structured clutter while avoiding fragmented detections and false alarms. Existing deep models have improved dense mask prediction, but their training objectives usually supervise the final mask without explicitly regularizing the geometry of individual foreground components. We present CGA-v2, a training-time component-geometry regularizer for MSHNet-style infrared small target detection. CGA-v2 derives deterministic center, boundary, scale, and peak targets from connected foreground components and attaches four auxiliary heads to an explicitly declared decoder feature. These heads are used only during training; inference remains unchanged and uses only the original final mask logit with a fixed threshold. This design separates component-level geometric supervision from test-time prediction changes, enabling a controlled paired comparison against the MSHNetOHEM baseline. The current manuscript draft reports the completed current-repository MSHNetOHEM seed42 epoch400 baseline and defines the evidence protocol for the paired CGA evaluation. CGA effectiveness claims, multi-seed robustness, and ablation conclusions are reserved until the matching epoch400 CGA runs and fixed-threshold evaluations are complete.
\end{abstract}

\section{Introduction}

Single-frame infrared small target detection (IRSTD) aims to recover small, dim target masks from infrared imagery. It is important for long-range sensing, maritime monitoring, aerial surveillance, and search systems where targets may occupy only a few pixels. The task differs from generic semantic segmentation because foreground evidence is sparse, object texture is weak, target boundaries are ambiguous, and structured background clutter can produce target-like responses. Classical local-contrast and low-rank formulations exploit this structure through handcrafted priors \citep{chen2014local,dai2017ript,zhang2019pstnn}, while modern neural methods learn dense predictors and multi-scale representations for the same sparse-object regime \citep{wang2019miss,dai2021acm,dai2021alcnet,li2023dnanet}.

Recent IRSTD models increasingly encode task-specific structure instead of treating the task as ordinary binary segmentation. Dense nested interaction preserves small objects through repeated high-low feature exchange \citep{li2023dnanet}; shape reconstruction emphasizes edges and target extent \citep{zhang2022isnet}; nested U-shaped feature aggregation improves local-global interaction \citep{wu2023uiunet}; point-supervised label evolution studies how target masks emerge from weak labels \citep{ying2023lesps}; and scale/location-sensitive supervision directly addresses target size and position \citep{liu2024sls}. These methods suggest that where a target component is, how large it is, how its boundary is represented, and how compactly it activates are part of the detection signal rather than incidental annotation details.

The remaining question is how to inject component-level geometry into a strong mask detector without changing the final inference path or making the evidence ambiguous. Adding post-processing, auxiliary predictions at test time, threshold search, or implicit tensor parsing can make an improvement difficult to attribute. If a method is presented as a training regularizer, the paper should show that the original final mask predictor improves under a fixed evaluation path, not that an extra test-time decision rule or a favorable threshold produces better numbers.

We propose CGA-v2, a component-geometry regularization framework for MSHNet-style IRSTD. CGA-v2 keeps the MSHNet final logit as the only inference output and adds four training-only auxiliary heads for center, boundary, scale, and peak supervision. The detector adapter must explicitly declare the final logit and the selected CGA feature, the wrapper must expose all four auxiliary logits, and paper-mode loss construction fails if any required head is absent. This fail-closed design makes the method boundary auditable: CGA-v2 is a training-time regularizer, not a different test-time detector.

This paper makes three contributions. First, it formulates component geometry as deterministic auxiliary supervision for infrared small target detection. For each connected foreground component, CGA-v2 constructs center, boundary, scale, and peak targets without using dataset-specific labels, image identifiers, or hard-clutter annotations. Second, it implements CGA-v2 as a training-only regularizer for MSHNet-style detectors. All reported predictions are produced only from the original final logit, with auxiliary heads ignored at inference. Third, it defines a controlled evidence protocol for evaluating whether component-geometry regularization improves the final mask predictor itself. The protocol fixes the dataset split, checkpoint epoch, threshold, final-logit evaluation path, and hard-clutter diagnostic split before computing paired deltas.

\section{Related Work}

\subsection{Infrared Small Target Detection}

IRSTD has a long history of methods that separate targets from structured backgrounds using local contrast, sparsity, and low-rank assumptions. Local contrast methods amplify target-background differences \citep{chen2014local}, while patch-tensor and nuclear-norm formulations model background redundancy and target sparsity \citep{dai2017ript,zhang2019pstnn}. These approaches make the small-target prior explicit, but they rely on handcrafted assumptions and often struggle when clutter and targets share local statistics.

Deep IRSTD models replace handcrafted separation with learned dense prediction while retaining task-specific inductive biases. ACM introduces asymmetric contextual modulation for exchanging global context and low-level detail \citep{dai2021acm}. ALCNet combines local contrast with attention \citep{dai2021alcnet}. DNANet develops dense nested interaction and contributes the NUDT-SIRST benchmark \citep{li2023dnanet}. Later models strengthen feature compensation, pyramid context, transformer-style interaction, or nested feature aggregation for complex backgrounds \citep{zhang2022fc3net,zhang2023agpcnet,chen2024tciformer,wu2023uiunet}. CGA-v2 is not positioned as a new backbone family. It regularizes a fixed MSHNet-style detector so that component geometry shapes training while the mask inference path remains unchanged.

\subsection{Shape-, Scale-, and Location-Aware Supervision}

Shape-, scale-, and location-aware supervision is the closest line of work to CGA-v2. ISNet shows that target shape and edge reconstruction matter for infrared small targets \citep{zhang2022isnet}. MSHNet with scale and location sensitivity is the closest architectural and supervision threat because it explicitly targets scale and position in the same task family \citep{liu2024sls}. PConv with SD Loss further studies spatial pixel distribution and scale-dependent loss weighting \citep{yang2025pconv}. CGA-v2 differs by decomposing regularization into four connected-component-derived targets and by making the evidence contract part of the method boundary: auxiliary heads are training-only, and all reported numbers must come from the final logit under a fixed threshold.

\subsection{Weak and Foundation-Model Supervision}
...
\vspace{0.4em}
Selected decoder feature $\rightarrow$ CGA auxiliary head $\rightarrow$ center, boundary, scale, peak logits \\
\vspace{0.4em}
...
CGA-v2 constructs deterministic supervision from the binary mask. Let $\mathcal{C}(y)$ be the set of eight-connected foreground components in a training mask. For each component $C \in \mathcal{C}(y)$, we compute an integer center from the rounded mean of foreground pixel coordinates:
\begin{equation}
\mu(C)=\mathrm{round}\left(\frac{1}{|C|}\sum_{p\in C}p\right).
\end{equation}
The center target $t_c$ marks $\mu(C)$ and dilates it by radius $r_c=2$. The boundary target $t_b$ is the binary difference between a radius-$r_b$ dilation and radius-$r_b$ erosion of the foreground mask, with $r_b=1$. The scale target $t_s$ assigns every pixel in component $C$ the value
\begin{equation}
...
The target builder uses only the current binary mask and fixed radii. It does not depend on dataset names, image identifiers, evaluation splits, hard-clutter labels, or external proposal generators. This design keeps the regularizer auditable and separates the method from the evidence protocol.

The four targets regularize complementary failure modes. The center target supplies a sparse localization cue that is less sensitive to the exact contour of tiny objects. The boundary target encourages the decoder representation to preserve target-background transitions, where false positives and fragmented masks often appear. The scale target provides a coarse component-size signal without introducing a separate scale classifier. The peak target encourages compact high-confidence activation around the component center. Since these maps are not used for prediction at test time, their effect on the reported metrics must occur through the learned final mask predictor.
...
At test time, CGA-v2 follows the same inference path as MSHNetOHEM. The model returns the final mask logit, the probability map is obtained by sigmoid, and the binary mask is produced by threshold 0.5. Auxiliary logits are not used for prediction, post-processing, confidence calibration, or failure filtering. This restriction is central to the regularization claim: any final result must come from the same final-logit evaluation path as the baseline.

\section{Experiments}

\subsection{Controlled Evaluation Protocol and Current Evidence}

We use a controlled paired protocol to ensure that any measured difference between MSHNetOHEM and MSHNetCGA is attributable to the training-time regularizer rather than to changes in inference or evaluation. Both methods are evaluated from epoch400 checkpoints, on the same NUDT-SIRST split, using the same final-logit sigmoid output and a fixed threshold of 0.5. The full test split is the primary evaluation split, while HC-Val is used only as a hard-clutter diagnostic split.

At the time of this manuscript draft, the current repository contains a completed MSHNetOHEM seed42 epoch400 baseline, while the paired MSHNetCGA seed42 epoch400 evaluation is still pending. Therefore, we report the verified baseline values and leave the CGA entries, paired deltas, multi-seed statistics, and ablations unclaimed until the matching current-repository evaluations are complete. Historical results from other repositories and fallback or mock regularizers are not admissible paper evidence.

\begin{table*}[t]
\centering
\caption{Current evidence gates in the repository. A pass here authorizes the protocol; it does not replace the missing paired CGA result.}
...
Dataset preflight & Pass & NUDT-SIRST split files and masks pass integrity checks. \\
HC-Val source audit & Pass & HC-Val source is accepted and has zero train overlap. \\
...
\subsection{Completed Baseline}

Table~\ref{tab:ohem-baseline} reports the completed current-repository MSHNetOHEM seed42 epoch400 baseline. These numbers are not a claim for CGA-v2. They establish the paired baseline that the matching CGA run must compare against.
...
\subsection{Failure Analysis Plan}
...
The final submission should include qualitative panels for at least three case types: target preservation under weak contrast, clutter suppression or fragmentation reduction, and failure cases where CGA increases false alarms or misses a target. Each panel should show the infrared image, ground-truth mask, MSHNetOHEM prediction, MSHNetCGA prediction, and optionally the auxiliary target maps for interpretation. Auxiliary logits should not be shown as predictions.
